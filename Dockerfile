...
RUN wget https://npm.taobao.org/mirrors/node/v8.11.3/node-v8.11.3-linux-x64.tar.gz && \
    tar -C /usr/local --strip-components 1 -xzf node-v8.11.3-linux-x64.tar.gz && \
    rm node-v8.11.3-linux-x64.tar.gz

WORKDIR /data/superloan-fe

# 安装cnpm
RUN npm install -g cnpm --registry=https://registry.npm.taobao.org

# 安装项目 npm 包
ADD package.json /data/superloan-fe/package.json
#RUN cnpm install

# 添加源代码
ADD . /data/superloan-fe
